...
/*   { name: 'Catalago', href: '#', current: true },
  { name: 'Contactanos', href: '#', current: false }, */
  { name: 'Agregar Producto', to: { name: 'productForm' }, current: false },
]
</script>

<template>
  <Disclosure as="nav" class="relative bg-gray-800/80 after:pointer-events-none 
  after:absolute after:inset-x-0 
       after:bottom-0 after:h-px after:bg-white/10" v-slot="{ open }">
             ...
             leave-from-class="transform scale-100" leave-to-class="transform opacity-0 scale-95">
              <MenuItems class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-gray-800 py-1 outline -outline-offset-1 outline-white/10">
                <MenuItem v-slot="{ active }">
                  <a href="#" :class="[active ? 'bg-white/5 outline-hidden' : '', 'block px-4 py-2 text-sm text-gray-300']">
                    Tu perfil</a>
                </MenuItem>
                <MenuItem v-slot="{ active }">
                  <a href="#" :class="[active ? 'bg-white/5 outline-hidden' : '', 'block px-4 py-2 text-sm text-gray-300']">
                ...
                <MenuItem v-slot="{ active }">
                  <button @click="userStore.logoutUser"  :class="[active ? 'bg-white/5 outline-hidden' : '', 'block px-4 py-2 text-sm text-gray-300']">
                    Cerrar sesión</button>
                </MenuItem>
              </MenuItems>
            </transition>
          </Menu>
